...
package ll.luolin.service.parserService;
 ...
 * ASF文件解析服务
 */
public class CHARACTERISTIC_POINTS_Service implements ASFService {
    private static CHARACTERISTIC_POINTS_Service instance;
    private final ConcurrentHashMap<String, List<NavPointModel>> fileCache = new ConcurrentHashMap<>();

    private CHARACTERISTIC_POINTS_Service() {
    }

    public static synchronized CHARACTERISTIC_POINTS_Service getInstance() {
        LogUtils.info("AsfService-AsfService-单例初始化");
        if (instance == null) {
            instance = new CHARACTERISTIC_POINTS_Service();
        }
        return instance;
    }
     ...
     */
    @Override
    public List<NavPointModel> parserFile(@NotNull File file) {
...
//                }
    ...
    public List<NavPointModel> parseAsfFiles(List<File> files) {
        ...
        for (File file : files) {
            try {
                List<NavPointModel> points = parserFile(file);
